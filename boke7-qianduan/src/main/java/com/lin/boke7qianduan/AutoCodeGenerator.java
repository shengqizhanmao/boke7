...
public class AutoCodeGenerator {
    ...
    public static void main(String[] args) {
        ...
        DataSourceConfig.Builder datasourceBuilder = new DataSourceConfig.Builder(
                "jdbc:mysql://localhost:3306/boke?useUnicode=true&characterEncoding=utf-8&zeroDateTimeBehavior=convertToNull&useSSL=false&serverTimezone=UTC",
                "root",
                "root"
        );
        ...
        generator.globalConfig(builder -> {
            ...
            builder.author("linSheng")
                    ...
                    .enableSwagger()
                    // 指定输出目录
                    .outputDir(PROJECT_ROOT_PATH + "/boke7-qianduan/src/main/java");
        });

        // 4、包配置
        generator.packageConfig(builder -> {
            // 父包名. ===========1.手动修改设置===========
            builder.parent("com.lin.boke7qianduan")
                    // 将要生成的模块名称. ===========2.手动修改设置===========
//                    .moduleName("employee")
                    // 设置生成的 控制层 文件夹名称
                    ...
                    .pathInfo(Collections.singletonMap(OutputFile.xml, PROJECT_ROOT_PATH + "/boke7-qianduan/src/main/resources/mapper"));
        });
        ...
        generator.strategyConfig(builder -> {
            ...
            // ===========3.手动修改设置。===========
            builder.addInclude("m_friends_user")
                    // 过滤表前缀，生成的类名会去掉这个前缀
